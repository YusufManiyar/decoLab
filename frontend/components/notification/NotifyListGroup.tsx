...
export const NotifyListGroup: React.FC<NotifyListGroupProps> = ({items, handleClick}) => {
    ...
    const handleAcceptClick = (item: NotificationType) => {
        try {
            sendMessage({type: "collaboration-accept", item});
        } catch(err) {
                ...
                >
                    <div className="flex flex-row items-start">
                        ...
                        <div className="flex flex-col ml-6">
                            <h1 className="font-poppins font-light text-md text-left"><span className="font-bold">{item.sender.name}</span> {item.notification.message}</h1>
